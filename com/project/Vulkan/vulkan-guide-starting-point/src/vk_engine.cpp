...
void VulkanEngine::init_vulkan()
{
	...
	// Store the gpu properties for later use
...
void VulkanEngine::init_pipelines()
{
	...
	{
		std::cout << "Error when building the colored mesh fragme	nt shader module" << std::endl;
	}
	else {
...
void VulkanEngine::init_scene()
{
	...
	// Add a bunch of triangles around it
	for (int x = -20; x <= 20; x++) {
		for (int y = -20; y <= 20; y++) {
...
void VulkanEngine::init_descriptors()
{
	...
	for (int i = 0; i < FRAME_OVERLAP; i++) {
		...
		// Alocate the descriptor set that will point to the object buffer
...
void VulkanEngine::draw_objects(VkCommandBuffer cmd, RenderObject* first, int count)
{
	...
	for (int i = 0; i < count; i++)
	{
		...
		if (object.material != lastMaterial) {
			...
			//bind the camera descriptor set when changing pipeline
...
size_t VulkanEngine::pad_uniform_buffer_size(size_t originalSize) {
	// Pads the size to the alignment boundary
